---
name: agent-react
description: "Use this when implementing a ReAct agent pattern, building an agent loop with thought-action-observation, or creating a tool-using agent from scratch"
---

# ReAct Agent Pattern

Implements the Reasoning + Acting pattern: Think -> Decide Action -> Execute -> Observe -> Repeat.
...
- **Act**: Parse action format (TOOLNAME[ARGS]) and execute
- **Observe**: Record tool output for next iteration
- History accumulates context for better reasoning
- Completion check prevents infinite loops
- For production, use LangChain/LangGraph implementations
